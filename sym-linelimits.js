    ...
    symbolVis.prototype.init = function (scope, elem) {
        ...
        function myCustomDataUpdateFunction(data) {
            ...
            if (data !== null && data.Data) {
                ...
                var hasFirstLimit = data.Data[1] ? true : false;
                var hasSecondLimit = data.Data[2] ? true : false;

                var firstLimit, secondLimit;
                firstLimit = secondLimit = null;

                if (hasFirstLimit) {
                    var firstLimits = data.Data[1];
                    if (scope.config.stringLabel2 == "") if (firstLimits.Label) scope.config.stringLabel2 = firstLimits.Label;
                    if (firstLimits.Values.length > 0) firstLimit = parseFloat(("" + firstLimits.Values[firstLimits.Values.length - 1].Value).replace(",", ""));
                }

                if (hasSecondLimit) {
                    var secondLimits = data.Data[2];
                    if (scope.config.stringLabel3 == "") if (secondLimits.Label) scope.config.stringLabel3 = secondLimits.Label;
                    if (secondLimits.Values.length > 0) secondLimit = parseFloat(("" + secondLimits.Values[secondLimits.Values.length - 1].Value).replace(",", ""));
                }

                for (var i = 0; i < itemsValues.Values.length; i++) {                                        
                    var horas = (new Date(itemsValues.Values[i].Time)).getHours();
                    var tiempo = new Date(itemsValues.Values[i].Time).toLocaleString().replace(",", "");       
