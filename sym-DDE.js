    ...
    var myEDcolumnDefinition = {
        ...
        getDefaultConfig: function() {
            return {
                ...
                labelunit: "",
                // Title
                titlegraph1:'DISPONIBILIDAD TURNO NOCHE',
                titlegraph2:'DISPONIBILIDAD TURNO DIA',
    ...
    symbolVis.prototype.init = function(scope, elem, timeProvider) {
        ...
        function myCustomDataUpdateFunction(data) {
            ...
            if (data !== null && data.Data) {
                ...
                if (scope.config.stringLabel1 == "")
                    if (firstTurn.Label) scope.config.stringLabel1 = firstTurn.Label;
                if (scope.config.stringUnits1 == "")
                    if (firstTurn.Units) scope.config.stringUnits1 = firstTurn.Units;

                if (scope.config.stringLabel2 == "")
                    if (secondTurn.Label) scope.config.stringLabel2 = secondTurn.Label;
                if (scope.config.stringUnits1 == "")
                    if (secondTurn.Units) scope.config.stringUnits2 = secondTurn.Units;

                let customDate = data.Data[4];
                let currentDay = new Date();
                
                let customStartDate = addDays(currentDay,-currentDay.getDate());            
        ...
        function fillDataArray(firstTurn, secondTurn, firstTurnReal, secondTurnReal, start, daysOfMonth, dataArray) {
            ...
            for (let dayIndex = 1; dayIndex <= (daysOfMonth + moreDays) ; dayIndex++) {
                ...
                let newDataObject = getNewDataObject(iterableDate.getDate(), floatFirstTurn, floatSecondTurn, total, media, [iterableDate.getMonth()+1]);
                dataArray.push(newDataObject);
                }
                else{
                    let newDataObject = getNewDataObject(iterableDate.getDate(), null, null, null, null, [iterableDate.getMonth()+1]);
                    dataArray.push(newDataObject);
                }
            }    
        }
        ...
        function getNewDataObject(dayIndex, firstTurnValue, secondTurnValue, total, media, month ) {
        ...
        function getNewChart(symbolContainerDiv, monthNow, scope, stringUnitsFirst, stringUnitsSecond, dataArray) {
            return AmCharts.makeChart(symbolContainerDiv.id, {
                ...
                "graphs": [{
                        ...
                        "bold": true,
                        "balloonText": scope.config.titlegraph1 + "</b><br />[[timestamp]]</b><br />[[turno1]] " + scope.config.labelunit + stringUnitsFirst,
                        "valueField": "turno1",
                        "valueAxis": "Axis1"
                    },
                    {
                        ...
                        "balloonText": scope.config.titlegraph2 + "</b><br />[[timestamp]]</b><br />[[turno2]] " + scope.config.labelunit + stringUnitsSecond,
                        "valueField": "turno2",
                        "valueAxis": "Axis1"
                    },
                    {
                        ...
                        "balloonText": scope.config.titlegraph3 + `</b><br />[[timestamp]]</b><br />[[${scope.config.graph3Value}]] ` + scope.config.labelunit + stringUnitsFirst,
                        "valueField": "total",
                        "labelText": `[[${scope.config.graph3Value}]]` + ' ' + scope.config.labelunit,
                        "fontSize": scope.config.fontSize,
                        "fillAlphas": 0,
