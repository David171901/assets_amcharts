(function (BS) {
    ...
    var definition = {
        ...
        typeName: 'parettotime',
        displayName: 'parettotime',
        ...
        function myCustomDataUpdateFunction(data) {
            if (data) {
                ...
                let sum = items.reduce(
                    (previousValue, currentValue) => previousValue + parseInt(currentValue.Value.split('||')[1]),
                    0
                );
                let accumulated_average = 0
                let colors = ["#3AB8A8","#34A496","#2F9084","#297C72", "#236960","#1D554F", "#17423D", "#102F2B",  "#0A1C1A", "#030908"]
                for (var i = 0; i < items.length; i++) {
                    if(!i==0) accumulated_average += (parseInt(items[i-1].Value.split('||')[1])/sum)*100
                    let item = items[i];
                    ...
                    let itemValue = (parseFloat(("" + (item.Value.split('||'))[1]).replace(",", "")))
                    let percent = parseFloat((accumulated_average+parseFloat(((item.Value.split('||')[1]/sum)*100).toFixed(scope.config.decimalPlaces))).toFixed(scope.config.decimalPlaces));
