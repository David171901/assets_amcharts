...
(function(CS) {
    var myEDcolumnDefinition = {
        typeName: "BarrasSobrepuestasV5",
        displayName: 'Barras sobrepuestas Data sin depurado',
        inject: ['timeProvider'],
    ...
    symbolVis.prototype.init = function(scope, elem, timeProvider) {
        ...
        function fillDataArray(firstTurn, secondTurn, firstTurnReal, secondTurnReal, start, daysOfMonth, dataArray) {
            ...
            for (let dayIndex = 1; dayIndex <= (daysOfMonth + moreDays) ; dayIndex++) {
                ...
                let newDataObject = getNewDataObject(iterableDate.getDate(), floatFirstTurn, floatSecondTurn, total, [iterableDate.getMonth()+1]);
                dataArray.push(newDataObject);
                }
                else{
                    let newDataObject = getNewDataObject(iterableDate.getDate(), null, null, null,[iterableDate.getMonth()+1]);
                    dataArray.push(newDataObject);
                }
            }     
        }
        ...
        function getNewDataObject(dayIndex, firstTurnValue, secondTurnValue, total, month ) {
            let todayDate = timeProvider.displayTime.end != "*" ? new Date(timeProvider.displayTime.end) : new Date();
            let currentMonth = todayDate.getMonth()+1;
            return {
